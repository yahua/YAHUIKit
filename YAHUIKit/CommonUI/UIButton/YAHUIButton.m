...
//#import "YAHUICore.h"
//#import "UIImage+YAHUI.h"
//#import "UIViewController+YAHUI.h"
#import "CALayer+YAHUI.h"
//#import "UIButton+YAHUI.h"
//#import "NSParagraphStyle+YAHUI.h"

#define ButtonTintColor RGB(49, 189, 243)
...
- (void)didInitialized {
    ...
    self.imagePosition = YAHUIButtonImagePositionLeft;
}
...
- (CGSize)sizeThatFits:(CGSize)size {
    ...
    switch (self.imagePosition) {
        case YAHUIButtonImagePositionTop:
        case YAHUIButtonImagePositionBottom: {
        ...
        case YAHUIButtonImagePositionLeft:
        case YAHUIButtonImagePositionRight: {
            // 图片和文字水平排版时，高度以文字或图片的最大高度为最终高度
            // 注意这里有一个和系统不一致的行为：当 titleLabel 为多行时，系统的 sizeThatFits: 计算结果固定是单行的，所以当 YAHUIButtonImagePositionLeft 并且titleLabel 多行的情况下，YAHUIButton 计算的结果与系统不一致
...
- (void)layoutSubviews {
    ...
    if (self.imagePosition == YAHUIButtonImagePositionTop || self.imagePosition == YAHUIButtonImagePositionBottom) {
        ...
        if (self.imagePosition == YAHUIButtonImagePositionTop) {
        ...
        imageFrame.origin.x = imageFrame.origin.x + _xOffsetImageAndTitle;
        titleFrame.origin.x = titleFrame.origin.x + _xOffsetImageAndTitle;
        
        self.imageView.frame = CGRectFlatted(imageFrame);
        self.titleLabel.frame = CGRectFlatted(titleFrame);
        
    } else if (self.imagePosition == YAHUIButtonImagePositionLeft || self.imagePosition == YAHUIButtonImagePositionRight) {
        ...
        if (self.imagePosition == YAHUIButtonImagePositionLeft) {
            switch (self.contentHorizontalAlignment) {
                case UIControlContentHorizontalAlignmentLeft:
                    imageFrame = isImageViewShowing ? CGRectSetX(imageFrame, contentEdgeInsets.left + self.imageEdgeInsets.left) : imageFrame;
...
- (void)setImagePosition:(YAHUIButtonImagePosition)imagePosition {
    _imagePosition = imagePosition;
    
    [self setNeedsLayout];
}

- (void)setXOffsetImageAndTitle:(CGFloat)xOffsetImageAndTitle {
    _xOffsetImageAndTitle = xOffsetImageAndTitle;
...
- (void)adjustsButtonHighlighted {
    if (self.highlightedBackgroundColor) {
        if (!self.highlightedBackgroundLayer) {
            ...
            [self.highlightedBackgroundLayer YAHUI_removeDefaultAnimations];
